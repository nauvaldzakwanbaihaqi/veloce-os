...
export function VeloceSystemHealth({
  ...
  const healthItems = [
    { label: "Thermal State", value: "38°C Normal", icon: Thermometer, color: "text-emerald-500" },
    { label: "Memory Pressure", value: "Low (45%)", icon: Activity, color: "text-blue-500" },
    { label: "Pool Connections", value: "12 / 100", icon: Database, color: "text-indigo-500" },
  ];

  return (
    <Card className="rounded-3xl border-0 shadow-sm shadow-blue-900/5 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md p-5 flex flex-col justify-between min-h-[220px]">
      {/* Header */}
      <div className="flex items-start justify-between">
        ...
        <Badge className="bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300 border-emerald-200/80 font-mono text-[10px]">
          {statusText}
        </Badge>
      </div>
      ...
      <div className="py-2 flex items-center justify-between gap-4">
        <div className="relative w-24 h-24 flex items-center justify-center shrink-0">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
            ...
            <span className="text-[8px] uppercase font-mono tracking-wider text-slate-400 dark:text-slate-500">
              Score
            </span>
          </div>
        </div>
        ...
        <div className="space-y-2 flex-1 min-w-0">
          {healthItems.map((item) => (
            <div key={item.label} className="flex items-center justify-between text-[11px] font-mono gap-1">
              ...
              <span className="font-semibold text-slate-900 dark:text-white shrink-0 text-right">{item.value}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="pt-2 border-t border-slate-100 dark:border-slate-800 space-y-1">
        <div className="flex justify-between text-[10px] font-mono text-slate-400 dark:text-slate-500">
          ...
          <span className="text-emerald-600 dark:text-emerald-400 font-bold">Passed (0 errors)</span>
        </div>
